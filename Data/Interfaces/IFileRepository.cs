...
public interface IFileRepository : IRepository
{
    ...
    Task<File?> FindFileByNameAsync(IDbConnection connection, Folder parent, string name);
    ...
    Task<IEnumerable<File>> EnumerateFilesByFolderAsync(IDbConnection connection, Folder parent);
    ...
    Task SaveFileAsync(IDbConnection connection, File file);
    ...
    /// <param name="connection">The connection to the database.</param>
    /// <returns>A task for async processing.</returns>
    Task MarkAllFilesAsUntouchedAsync(IDbConnection connection);
    ...
    /// <returns>A task for async processing.</returns>
    Task TouchFileAsync(IDbConnection connection, File file);
    ...
    /// <param name="connection">The connection to the database.</param>
    /// <returns>A task for async processing.</returns>
    Task RemoveAllDuplicateMarks(IDbConnection connection);
    ...
    Task MarkFileAsDuplicate(IDbConnection connection, File file);
    ...
    Task<IEnumerable<DuplicateFiles>> FindDuplicateFilesAsync(IDbConnection connection);
    ...
    Task<IEnumerable<string>> FindHashesOfDuplicateFilesAsync(IDbConnection connection);
    ...
    Task<IEnumerable<File>> EnumerateDuplicateFiles(IDbConnection connection);
    ...
    /// <returns>An enumeration of files that are duplicates of the given file.</returns>
    Task<IEnumerable<File>> EnumerateDuplicatesWithHash(IDbConnection connection, string hash);
